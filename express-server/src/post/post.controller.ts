...
export class PostController implements Controller {
	...
	private initImageService(): void {
		this.imageService = new ImageUploader(
			this.baseUploadPath,
			this.sizesConfigs
		);
	}
		...
		try {
			...
			for (const post of foundPosts) {
				if (req.query.includeComments == 'true') {
					const posts = await post.populate(<PopulateOptions>{
						path: 'comments',
						options: {
					...
					await posts.populate('commentsAsParentCount');
					await posts.populate('commentsCount');
				}
				if (req.query.includeReactions == 'true') {
					...
					await posts.populate('reactionsCount');
				}
			}
	...
	private createPost = async (req: RequestUser, res: Res, next: Next) => {
		...
		try {
			const [thumbPath, imagePath] = await this.uploadPostImage(
				req.file,
				'url'
			);
	...
	private deletePost = async (req: RequestUser, res: Res, next: Next) => {
		...
		try {
			await Promise.all([
				this.imageService.removeAssociatedImages(
					foundPost.image,
					this.sizesConfigs
				),
				foundPost.remove(),
	 ...
	 */
	protected async uploadPostImage(
		file: any,
		key?: string
	): Promise<string[]> {
		...
		const images = await this.imageService.uploadImage({
			file,
			options,
		} as UploadImageDto);
		const thumbPath = images.find(
			(img) => img.sizeLabel === SizeLabel.THUMB
		)[key || 'name'];
		const imagePath = images.find(
			(img) => img.sizeLabel === SizeLabel.MEDIUM
		)[key || 'name'];
